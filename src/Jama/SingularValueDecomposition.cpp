...
/** Construct the singular value decomposition
 ...
 SingularValueDecomposition::SingularValueDecomposition (Matrix Arg) {
	...
	int nu = std::min(m,n);
	s = new double[std::min(m+1,n)];
    //动态开辟空间
    U = new double*[m]; //开辟行
    V = new double*[m];
	for(int i = 0; i < m; i++){
		U[i] = new double[n]; //开辟列
        V[i] = new double[n];
    }
	...
	for (int k = 0; k < std::max(nct,nrt); k++) {
		if (k < nct) {
			...
			s[k] = 0;
			for (int i = k; i < m; i++) {
			   s[k] = jama::util::Maths::hypot(s[k],A[i][k]);
			}
			if (s[k] != 0.0) {
			   if (A[k][k] < 0.0) {
				  s[k] = -s[k];
	  ...
	  while (p > 0) {
		 ...
		 switch (kase) {
			...
			case 1: {
			   ...
			   e[p-2] = 0.0;
			   for (int j = p-2; j >= k; j--) {
				  double t = jama::util::Maths::hypot(s[j],f);
				  double cs = s[j]/t;
				  double sn = f/t;
				  s[j] = t;
				  if (j != k) {
			...
			case 3: {
			   ...
			   for (int j = k; j < p-1; j++) {
				  double t = jama::util::Maths::hypot(f,g);
				  double cs = f/t;
				  double sn = g/t;
				  if (j != k) {
					 e[j-1] = t;
...
Matrix SingularValueDecomposition::getS () {
 ...
 /** Return the one-dimensional array of singular values
   ...
   */
double* SingularValueDecomposition::getSingularValues () {
	  return s;
} 
...
Matrix SingularValueDecomposition::getU () {
	return *(new Matrix(U,m,std::min(m+1,n)));
}   
...
int SingularValueDecomposition::get_s_size() {
